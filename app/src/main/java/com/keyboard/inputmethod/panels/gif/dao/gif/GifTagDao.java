...
public final class GifTagDao {
	...
	public void createTable(SQLiteDatabase db,final String tag){
		...
		String sql="CREATE TABLE IF NOT EXISTS "+tag.replace(" ","")+ " ("+_ID+" INTEGER PRIMARY KEY AUTOINCREMENT,"
				+ MEDIA_ID + " TEXT,"
				+ URL + " TEXT,"
				+ GIF + " TEXT,"
				+ MP4 + " TEXT"
	...
	public void insert(final String tag,List<GifItem> items){
		...
		try{
			...
			final String sql="insert into "+tag.replace(" ","")+INSERT;
	...
	public void clearAll(final String tag){
		...
		database.beginTransaction();
		try{
			final String sql="delete from " +tag.replace(" ","");
			createTable(database,tag.replace(" ",""));
	...
	public List<GifItem> getAll(final String tag){
		...
		try{
			createTable(database,tag.replace(" ",""));
			final String sql="select * from " +tag.replace(" ","");
