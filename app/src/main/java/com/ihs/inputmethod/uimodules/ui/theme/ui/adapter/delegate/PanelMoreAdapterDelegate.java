...
public final class PanelMoreAdapterDelegate extends AdapterDelegate<List<ThemePanelModel>> {

	private int spanCount;

	public PanelMoreAdapterDelegate(int spanCount) {
		this.spanCount = spanCount;
	}

	@Override
	protected boolean isForViewType(@NonNull List<ThemePanelModel> items, int position) {
		return items.get(position).isMoreButton;
	}

	@NonNull
	@Override
	protected RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent) {
		return new PanelMoreViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_panel_theme_more, parent, false));
	}

	@Override
	protected void onBindViewHolder(@NonNull List<ThemePanelModel> items, int position, @NonNull RecyclerView.ViewHolder holder) {
		final PanelMoreViewHolder viewHolder = (PanelMoreViewHolder) holder;
		...
		if (model.isCustomThemeInEditMode) {
			...
			Drawable normalStatusDrawable = HSKeyboardThemeManager.getCurrentTheme().getStyledDrawableFromResources("theme_more_icon");
			Drawable pressedStatusDrawable = HSKeyboardThemeManager.getCurrentTheme().getStyledDrawableFromResources("theme_more_icon_pressed");
