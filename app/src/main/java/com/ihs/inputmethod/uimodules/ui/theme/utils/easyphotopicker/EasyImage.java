package com.ihs.inputmethod.uimodules.ui.theme.utils.easyphotopicker;
...
public class EasyImage implements EasyImageConfig {
    ...
    public static void openGalleryStable(Activity activity,int type){
        try {
            openGallery(activity, type);
        } catch (Exception e) {
            e.printStackTrace();
            openDocuments(activity, type);
        }
    }
}